...
exports.updateDailyAggregation = updateDailyAggregation;
exports.getSummary = getSummary;
exports.getMonthlyBreakdown = getMonthlyBreakdown;
exports.getMonthlyDetail = getMonthlyDetail;
...
// ============================================================
async function updateDailyAggregation(eventType, data) {
    ...
    await prisma_1.prisma.dailyAggregation.upsert({
...
async function getSummary(filters) {
    const where = { isSandbox: filters?.includeSandbox ? undefined : false };
    ...
    const approved = await prisma_1.prisma.eventProcessed.findMany({
        where: {
            ...where,
    ...
    const refused = await prisma_1.prisma.eventProcessed.count({
        where: {
            ...where,
            eventType: { in: ['payment.refused', 'recharge.refused'] },
        },
    });
    const received = await prisma_1.prisma.eventProcessed.count({
        where: {
            ...where,
...
async function getMonthlyBreakdown(months = 12) {
    ...
    const aggregations = await prisma_1.prisma.dailyAggregation.findMany({
        where: {
...
async function getMonthlyDetail(month) {
    // month = 'YYYY-MM'
    const startDate = new Date(`${month}-01T00:00:00.000Z`);
    const endDate = new Date(startDate);
    endDate.setMonth(endDate.getMonth() + 1);
    const APPROVED = ['payment.approved', 'recharge.completed', 'charge.paid'];
    const REFUSED = ['payment.refused', 'recharge.refused'];
    const [approved, refused] = await Promise.all([
        prisma_1.prisma.eventProcessed.findMany({
            where: {
        ...
        prisma_1.prisma.eventProcessed.count({
            where: {
                processedAt: { gte: startDate, lt: endDate },
