...
export default function SellerOrderChat({
  ...
  const fetchMessages = () => {
    ...
    setError(null);
    api
      .get(`/marketplace/seller-order/${sellerOrderId}/chat`)
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const { data } = await api.post(`/marketplace/seller-order/${sellerOrderId}/chat`, {
        content: text,
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { data } = await api.post(
        `/marketplace/seller-order/${sellerOrderId}/chat/attachment`,
        formData,
  ...
  const handleDownloadAttachment = async (messageId: string, fileName: string) => {
    try {
      ...
      const url = `${getApiBaseUrl()}/marketplace/seller-order/chat/attachment/${messageId}`;
    ...
    return (
      <div className="rounded-xl border border-gray-700/50 bg-gray-800/50 p-6">
        <div className="flex items-center gap-2 text-gray-400 mb-4">
          ...
          <span>Conversa com {sellerName || 'vendedor'}</span>
        </div>
    ...
    <div className="rounded-xl border border-gray-700/50 bg-gray-800/50 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-700/50 bg-gray-900/30">
        ...
        <span className="font-medium text-white">
          Conversa com {sellerName || 'vendedor'}
        </span>
      </div>
